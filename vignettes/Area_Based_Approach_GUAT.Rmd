...
polyLevel0 <- sf::st_read( paste0(mainDirroot,"/data-raw/admin/GTM/gtm_admbnda_adm0_ocha_conred_20190207.shp"), quiet = TRUE)
polyLevel1 <- sf::st_read( paste0(mainDirroot,"/data-raw/admin/GTM/gtm_admbnda_adm1_ocha_conred_20190207.shp"), quiet = TRUE)
polyLevel2 <- sf::st_read( paste0(mainDirroot,"/data-raw/admin/GTM/gtm_admbnda_adm2_ocha_conred_20190207.shp"), quiet = TRUE)

# polyLevel0 <- raster::getData('GADM', country = 'GTM', level = 0)
...
# ## Fetch shapefiles 
# 
# polyLevel1 <- raster::getData('GADM', country = 'GTM', level = 1)
# ## Convert the spatial file to sf to increase speed and ease of plotting
polyLevel1_sf <- sf::st_as_sf(polyLevel1) %>%
  rmapshaper::ms_simplify()
# 
# 
# polyLevel2 <- raster::getData('GADM', country = 'GTM', level = 2)
# ## Convert the spatial file to sf to increase speed and ease of plotting
...
population_under5 <- rhdx::pull_dataset("guatemala-high-resolution-population-density-maps-demographic-estimates") %>%
  get_resource(4) %>%
  read_resource() %>%
  as("Raster")
...
population_60 <- rhdx::pull_dataset("guatemala-high-resolution-population-density-maps-demographic-estimates") %>%
  get_resource(6) %>%
  read_resource() %>%
  as("Raster")
...
reproductive_women <- rhdx::pull_dataset("guatemala-high-resolution-population-density-maps-demographic-estimates") %>%
  get_resource(12) %>%
  read_resource() %>%
  as("Raster")
...
youth <- rhdx::pull_dataset("guatemala-high-resolution-population-density-maps-demographic-estimates") %>%
  get_resource(14) %>%
  read_resource() %>%
  as("Raster")
...
# Draw the map
# Set a color palette
pal <- mapsf::mf_get_pal(n = 9, palette = "RdPu", rev = TRUE)
```
...
#        add = TRUE)
# # Set a layout
# mapsf::mf_title(txt = "Population Density in Guatemala", 
#                 fg = "#FFFFFF")
...
#   
#       labs(title = "Population Density in Guatemala" ,
...
md <- subset(movement_range,country %in% c( "GTM" #, "HND", "MEX","BLZ", "GTM"
                                            ))
...
#          title = title)
#   ggsave(glue({'anim/GTM-{target_ds}-staying put.png'}), g, scale = 1.0,
#          width = unit(4, 'cm'),
#          height = unit(4, 'cm'))
#   g
# }
...
acledsp <- sf::st_as_sf(acled.data, coords = c("longitude", "latitude"), crs = 4326)  %>%
 ...
 sf::st_join(., polyLevel2_sf, join = sf::st_within) 
...
# Set a layout
mapsf::mf_title(txt = "Battles in Guatemala", 
                fg = "#FFFFFF")
...
# Set a layout
mapsf::mf_title(txt = "Explosions & Remote Violence in Guatemala", 
                fg = "#FFFFFF")
mapsf::mf_credits(txt = "Source: ACLED - aggregated data per level 2 since 2018",
                   bg = "#ffffff80")
```
...
# Set a layout
mapsf::mf_title(txt = "Protests in Guatemala", 
                fg = "#FFFFFF")
mapsf::mf_credits(txt = "Source: ACLED - aggregated data per level 2 since 2018",
                   bg = "#ffffff80")
```
...
# Set a layout
mapsf::mf_title(txt = "Strategic developmentsin Guatemala", 
                fg = "#FFFFFF")
mapsf::mf_credits(txt = "Source: ACLED - aggregated data per level 2 since 2018",
                   bg = "#ffffff80")
```
...
# Set a layout
mapsf::mf_title(txt = "Violence against Civilians in Guatemala", 
                fg = "#FFFFFF")
mapsf::mf_credits(txt = "Source: ACLED - aggregated data per level 2 since 2018",
                   bg = "#ffffff80")
```
...
# Set a layout
mapsf::mf_title(txt = "Riots in Guatemala", 
                fg = "#FFFFFF")
mapsf::mf_credits(txt = "Source: ACLED - aggregated data per level 2 since 2018",
                   bg = "#ffffff80")
```
...
# 
# # Set a color palette
# pal <- mapsf::mf_get_pal(n = 9, palette = "Burg", rev = TRUE)  
# 
...
# # Set a layout
# mapsf::mf_title(txt = "Fatalities in Guatemala", 
#                fg = "#FFFFFF")
...
# 
# files <- unzip(zip_file_path, list = TRUE)
# files <- grep("gtm_relative_wealth_index", files$Name, value = TRUE)
# 
...
# find points within polygons
wealth_in_level2 <-
       sf::st_join(wealthsp, polyLevel2_sf, join = sf::st_within) %>%
       sf::st_drop_geometry() %>%
                    group_by(ADM2_PCODE) %>%
...
# Set a layout
mapsf::mf_title(txt = "Relative Wealth Index in Guatemala", 
                fg = "#FFFFFF")
mapsf::mf_credits(txt = "Source: Facebook Data For Good", 
           bg = "#ffffff80")
...
#   filter(str_detect(user_loc, "GTM")) %>% 
#   group_by(user_loc ) %>% 
...
#country_sci2 <-  country_sci  %>% 
              ...
              mutate( ADM1_PCODE = str_replace_all(ADM1_PCODE, "GTM", "GT"))%>% 
              mutate( ADM1_PCODE = ifelse (nchar( ADM1_PCODE) <4, str_replace_all(ADM1_PCODE, "GT", "SV0"), ADM1_PCODE )  )
                              

polyLevel2_sf <- polyLevel2_sf %>% 
  dplyr::left_join(country_sci ,  by = "ADM1_PCODE")
...
country_sci <- rename(sci_dat, sci=scaled_sci)  %>% 
  filter(str_detect(user_loc, "GTM")) %>% 
  filter(str_detect(fr_loc, "GTM")) %>% 
              rename(  ADM1_PCODE = user_loc) %>% 
              mutate( ADM1_PCODE = str_replace_all(ADM1_PCODE, "GTM", "GT"))%>% 
              mutate( ADM1_PCODE = ifelse (nchar( ADM1_PCODE) <4, str_replace_all(ADM1_PCODE, "SV", "GT0"), ADM1_PCODE )  )
...
#            subtitle = "Guatemala, as of August 2020",
#            x = "",
#            y = "",
#            fill = "SCI", 
#            caption = "Source: Facebook Data For Good ") + 
...
shapes_in <- gadm1 %>% 
  sf::st_transform(crs=4326)  %>% 
 # sf::st_transform(crs("+proj=robin +lon_0=0 +x_0=0 +y_0=0 +ellps=WGS84 +datum=WGS84 +units=m +no_defs"))  %>% 
  filter(GID_0 %in% c("GTM","GTM", "HND", "MEX")) 
...
#            subtitle = "Guatemala to Honduras, Guatemala & Mexico, as of August 2020",
#            x = "",
#            y = "",
#            fill = "SCI", 
#            caption = "Source: Facebook Data For Good ") + 
...
#            subtitle = "Guatemala to USA, Honduras, Guatemala & Mexico, as of August 2020",
#            x = "",
#            y = "",
#            fill = "SCI", 
#            caption = "Source: Facebook Data For Good ") + 
...
[FAO-ASI](http://www.fao.org/giews/earthobservation/country/index.jsp?code=GTM&lang=en))
...
# gdal_translate -of Gtiff  -outsize 5000 5000 "WMS:https://io.apps.fao.org/geoserver/wms/ASIS/HDF/v2?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&LAYERS=HDF&SRS=EPSG:4326&BBOX=-90.3159,12.8091,-87.4539,14.6022" asi2_GTM.tiff


## save locallly a geotiff from the WMS server
# system("gdal_translate -of Gtiff  -outsize 5000 5000 \"WMS:https://io.apps.fao.org/geoserver/wms/ASIS/HDF/v2?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&LAYERS=HDF&SRS=EPSG:4326&BBOX=-90.3159,12.8091,-87.4539,14.6022\" /data-raw/fao/asi2_GTM.tiff")  

# load raster in an R object called 'DEM'
ASIpath <- paste0(mainDirroot,"/data-raw/fao/asi2_GTM.tiff")
#ASI <- raster(ASIpath) 
# 
# GDALinfo(ASIpath)
# 
...
leaflet() %>% 
  addTiles() %>% 
  # 8/15.845/-90.580
  setView(lat = 15.845, lng = -90.580, zoom = 8) %>% 
  addWMSTiles(
    wmshist,
    layers = layerhist,
...
# Set a layout
mapsf::mf_title(txt = "Areas Characterisation - Guatemala", 
                fg = "#FFFFFF")
